...
Run Create_conda_env.sh shell script file 
```bash
#!/bin/bash --login
...
Then for running the train file run below code bash script 
```bash
#!/bin/bash
set -e
log() {
...
This script is designed for evaluating machine learning models using various metrics and options. Below are the available command-line arguments along with their descriptions:

1. `--fold`: Specifies the number of folds for cross-validation. Default value is set to 5.

2. `--model`: Specifies the type of model to be evaluated. Choose from "lr" (Logistic Regression), "dt" (Decision Tree), "rf" (Random Forest), or "xgb" (XGBoost). Default is set to "lr".

3. `--logs`: Specifies the file path to save logs. Default is set to None.

4. `--confusion_matrix`: Boolean flag indicating whether to display the confusion matrix. Default is set to False.

5. `--metric`: Specifies the evaluation metric to be used. Choose from "f1_score", "roc_auc", "precision", "recall", "accuracy", or "f1_weighted". Default is set to 'roc_auc'.

6. `--auc_plot`: Boolean flag indicating whether to plot the ROC AUC curve. Default is set to False.

#### Example Usage:

```
python train.py --fold 10 --model lr --auc_plot False --logs lr_with_quantile --metric roc_auc
```
...
4. **Modeling:**
   ...
   **we might want to consider techniques such as class weighting, `resampling method`s (e.g., `oversampling` the minority class), or using `evaluation metrics` that are sensitive to class imbalance to ensure that our 
   model effectively learns from both classes and generalizes well**
...
```
![](https://github.com/shivamkc01/Bank_Standard_bank_churn/blob/main/plots/roc_auc_curve/roc_curve_training_testing_dt.png)
...
```
![](https://github.com/shivamkc01/Bank_Standard_bank_churn/blob/main/plots/roc_auc_curve/roc_curve_training_testing_xgb.png)

6. **Model Evaluation:**
   - Evaluated models using precision, recall, and ROC-AUC metrics.
   - Selected XGBoost as the best-performing model based on high ROC-AUC scores but for best interpertibility I would say we can consider Logistic or Deicision Tree.
...
## Future Work:
- **Advanced Techniques:** Exploration of advanced machine learning techniques and real-time data integration to improve model accuracy and responsiveness use Neural Network or use order Advance ML models like Catboost, LightGBM model.
